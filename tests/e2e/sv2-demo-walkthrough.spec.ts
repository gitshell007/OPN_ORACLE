...
test.describe("SV2-E2E-CAMINO demo walkthrough (oracle-dev UI)", () => {
  test("guion completo: portada → documentos → preguntar → informes + layout", async ({
    ...
    // Banner de degradación no debe inventarse: camino healthy (failed=[]) → sin aviso.
    // Si reaparece, o hay degradación real (fallo de cobertura/publisher) o regresión del hardcode.
    const degradedBanner = page
      .getByRole("status")
      .filter({ hasText: /Respuesta degradada/i });
    await expect(
      degradedBanner,
      "sin banner «Respuesta degradada» en el camino healthy del demo",
    ).toHaveCount(0);

    // Re-assert layout after answer: long UUIDs / pre.answer-block must not widen body.
