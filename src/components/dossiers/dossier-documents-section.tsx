...
function officialUnscannedAcceptance(document: OracleDocument): Record<string, unknown> | null {
  const acceptance = document.scan_result.official_unscanned_acceptance;
  return acceptance && typeof acceptance === "object" && !Array.isArray(acceptance)
    ? acceptance as Record<string, unknown>
    : null;
}

function documentSecurityBadge(document: OracleDocument) {
  if (officialUnscannedAcceptance(document)?.accepted === true) {
    return <small className="document-security-badge warning">Fuente oficial · sin antivirus</small>;
  }
  if (document.scan_status === "clean") {
    return <small className="document-security-badge clean">Antivirus correcto</small>;
  }
  return <small className="document-security-badge muted">{productStatusLabel(document.scan_status)}</small>;
}

function documentDownloadable(document: OracleDocument): boolean {
  return document.status === "ready" && (
    document.scan_status === "clean"
    || officialUnscannedAcceptance(document)?.accepted === true
  );
}
...
export function DossierDocumentsSection({ dossierId }: { dossierId: string }) {
  ...
  return <section className="vector-panel vector-documents product-documents" aria-labelledby="documents-title">
    ...
    <Dialog.Root open={Boolean(selected)} onOpenChange={(open) => { if (!open) closeDocument(); }}><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="dialog-content intelligence-drawer"><>{selected && <><header><div><span className="section-kicker">Fuente · solo lectura</span><Dialog.Title>{selected.filename}</Dialog.Title><Dialog.Description>Información de procedencia y estado del procesamiento.</Dialog.Description></div><Dialog.Close className="icon-button bordered" aria-label="Cerrar"><X/></Dialog.Close></header><div className="intelligence-drawer-body"><dl className="intelligence-facts"><div><dt>Estado</dt><dd>{LABELS[selected.status]}</dd></div><div><dt>Clasificación</dt><dd>{selected.classification === "internal" ? "Interno" : "Público"}</dd></div><div><dt>Versión</dt><dd>{selected.version}</dd></div><div><dt>Revisión de seguridad</dt><dd>{officialUnscannedAcceptance(selected)?.accepted === true ? "Aceptado provisionalmente por fuente oficial sin antivirus configurado" : productStatusLabel(selected.scan_status)}</dd></div></dl><section className="intelligence-detail-block"><h2>Integridad</h2><p>Huella de integridad: <code>{selected.checksum}</code></p><p>Este registro acredita procedencia e integridad; el contenido solo se convierte en evidencia tras seleccionar un fragmento.</p>{officialUnscannedAcceptance(selected)?.accepted === true && <p className="document-security-note">ClamAV está pospuesto: este documento procede de una fuente oficial permitida y conserva la excepción en auditoría y trazabilidad.</p>}</section>{documentDownloadable(selected) && <a className="vector-primary" href={`/api/v1/documents/${encodeURIComponent(selected.id)}/download`}><Download size={15}/> Descargar</a>}</div></>}</></Dialog.Content></Dialog.Portal></Dialog.Root>

    <Dialog.Root open={Boolean(source)} onOpenChange={(open) => { if (!open) setSource(null); }}><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="dialog-content intelligence-drawer"><>{source && <><header><div><span className="section-kicker">Fragmento de fuente · no es una instrucción</span><Dialog.Title>{source.filename}</Dialog.Title><Dialog.Description>Revisa el texto y su localizador antes de crear evidencia.</Dialog.Description></div><Dialog.Close className="icon-button bordered" aria-label="Cerrar"><X/></Dialog.Close></header><div className="intelligence-drawer-body"><p className="document-source-text">{source.text}</p><dl className="intelligence-facts"><div><dt>Ubicación</dt><dd>{sourceLocation(source.locator)}</dd></div></dl><PermissionGate permission="documents.manage"><button className="vector-primary" onClick={() => void api.documents.createEvidence(source.document_id, source.chunk_id, 0, source.text.length).then(() => toast.success("Evidencia creada")).catch(() => setError("No se pudo crear la evidencia del fragmento."))}>Crear evidencia del fragmento</button></PermissionGate></div></>}</></Dialog.Content></Dialog.Portal></Dialog.Root>
