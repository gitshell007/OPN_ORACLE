...
export function VectorDocuments({ dossierId }: { dossierId: string }) {
  ...
  return (
    <section className="vector-panel vector-documents" aria-labelledby="documents-title">
      ...
      {selected && <div className="evidence-drawer" role="dialog" aria-modal="true" aria-labelledby="evidence-source-title"><button className="evidence-backdrop" aria-label="Cerrar fuente" onClick={() => setSelected(null)}/><aside><header><div><span className="section-kicker">Fuente no confiable · solo lectura</span><h3 id="evidence-source-title">{selected.filename}</h3></div><button className="icon-button bordered" aria-label="Cerrar" onClick={() => setSelected(null)}>×</button></header><p>{selected.text}</p><dl><div><dt>Ubicación</dt><dd>{sourceLocation(selected.locator)}</dd></div></dl><AsyncActionButton className="vector-primary" onClick={() => void api.documents.createEvidence(selected.document_id, selected.chunk_id, 0, selected.text.length).then(() => toast.success("Evidencia creada")).catch(() => toast.error("Selecciona un rango válido en la fuente."))}>Crear evidencia del fragmento</AsyncActionButton></aside></div>}
    </section>
  );
}
