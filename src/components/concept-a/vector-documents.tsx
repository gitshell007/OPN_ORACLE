...
export function VectorDocuments({ dossierId }: { dossierId: string }) {
  ...
  return (
    <section className="vector-panel vector-documents" aria-labelledby="documents-title">
      ...
      {!!results.length && <div className="document-results" aria-live="polite"><h3>Resultados ({results.length})</h3>{results.map((result) => <button key={result.chunk_id} onClick={() => setSelected(result)}><strong>{result.filename}</strong><span>{result.snippet}</span><small>{Object.entries(result.locator).map(([key,value]) => `${key}: ${String(value)}`).join(" · ")}</small></button>)}</div>}
      {selected && <div className="evidence-drawer" role="dialog" aria-modal="true" aria-labelledby="evidence-source-title"><button className="evidence-backdrop" aria-label="Cerrar fuente" onClick={() => setSelected(null)}/><aside><header><div><span className="section-kicker">Fuente no confiable · solo lectura</span><h3 id="evidence-source-title">{selected.filename}</h3></div><button className="icon-button bordered" aria-label="Cerrar" onClick={() => setSelected(null)}>×</button></header><p>{selected.text}</p><dl>{Object.entries(selected.locator).map(([key,value]) => <div key={key}><dt>{key}</dt><dd>{String(value)}</dd></div>)}</dl><button className="vector-primary" onClick={() => void api.documents.createEvidence(selected.document_id, selected.chunk_id, 0, selected.text.length).then(() => toast.success("Evidencia creada")).catch(() => toast.error("Selecciona un rango válido en la fuente."))}>Crear evidencia del fragmento</button></aside></div>}
    </section>
  );
}
